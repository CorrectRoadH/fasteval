...
You select an agent by name — `--agent <name>` — not by URL. The URL, API key, or CLI invocation is the adapter's private configuration. fasteval never sees it.

## Why `--agent` and not `--url`
...
MY_BOT_URL=http://localhost:3000  npx fasteval weather --agent my-bot   # local
MY_BOT_URL=https://prod.example.com npx fasteval weather --agent my-bot # production
```
...
interface AgentCapabilities {
  ...
  workspace?: boolean;           // works on a filesystem → t.diff / t.sandbox
}
...
| `workspace` *(sandbox agents)* | `t.sandbox`, `t.diff`, `t.transcript`, `t.fileChanged()`, `t.testsPassed()`, `t.scriptPassed()` |
...
Use them from the CLI: `--agent claude-code`, `--agent codex`, `--agent bub`.

## Agent × Sandbox orthogonality

The `--agent` and `--sandbox` flags are fully independent:

```shell
--agent <name>       selects which system under test to connect to
...
Add your adapter to `fasteval.config.ts` and it becomes available to all evals and to the `--agent` flag:
...
export default defineConfig({
  agents: [myAgent, supportBot],   // register your adapters
  defaultAgent: "my-agent",        // used when --agent is omitted
});
```

Built-in coding agents (`claude-code`, `codex`, `bub`) are available without registration — use them directly with `--agent <name>`.
