...
description: "Learn how niceeval connects to any AI through named agent adapters. Remote agents wrap HTTP or in-process calls; sandbox agents run CLI tools in isolation."
---
...
interface AgentContext {
  ...
  readonly sandbox?: Sandbox;            // only present for sandbox agents (backend chosen by experiment's `sandbox` field)
...
experiment.sandbox   selects where sandbox agents run — dockerSandbox() / vercelSandbox() / e2bSandbox()
                      (config.sandbox is the project-wide fallback: exp.sandbox ?? config.sandbox)
```

Any sandbox agent works with any sandbox backend. `claude-code` can run in Docker or Vercel; the same Docker sandbox can host `claude-code` or `bub`. The runner prepares a `Sandbox` handle and passes it through `ctx.sandbox` — agent and sandbox interact only through that interface. Remote agents (`defineAgent`) ignore `sandbox` entirely. There is no CLI flag for sandbox selection and no auto-detection — a sandbox-capable agent's experiment (or the project config, as a fallback) must set `sandbox` explicitly, or niceeval throws when it tries to create one.
