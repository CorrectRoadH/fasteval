---
title: "fasteval scoring: assertions, judge calls, and verdicts"
sidebarTitle: "Scoring"
description: "fasteval scoring: five mechanisms — value assertions, scoped assertions, LLM-as-judge, test-as-scoring, and efficiency checks. Gate vs soft severity."
---

Scoring is the process of taking everything an agent did during an eval — every message, tool call, file change, and token spent — and folding it into a single, actionable verdict. fasteval gives you five scoring mechanisms that complement each other: some check values immediately, some assess the whole run after it completes, some ask a language model to judge open-ended quality, some execute tests inside the sandbox, and some measure efficiency. All five produce the same `Assertion` type, and all five feed into the same verdict rules.

## The five scoring mechanisms

<CardGroup cols={2}>
  <Card title="1. Value assertions" icon="equals">
    `t.check(value, matcher)` and `t.require(value, matcher)` evaluate a specific value immediately against a matcher from `fasteval/expect`. Use these for facts you can verify inline.
  </Card>
  ...
  <Card title="3. LLM-as-judge" icon="gavel">
    `t.judge.factuality()`, `t.judge.closedQA()`, `t.judge.summarizes()`, and `t.judge.score()` ask a separate evaluator model to score open-ended output. The judge model is fully independent from the agent under test.
  </Card>
...
<Tabs>
  <Tab title="gate">
    A gate assertion is a hard requirement. If it fails, the entire eval is immediately classified as `failed` — regardless of how well every other assertion scored. Use gate for facts that must be true: "the agent called the correct tool", "the response parsed as valid JSON", "no shell commands errored."
    
    Most matchers in `fasteval/expect` (`includes`, `equals`, `matches`, `satisfies`) default to gate. Scoped assertions like `t.succeeded()` and `t.calledTool()` also default to gate.
  </Tab>
  <Tab title="soft">
    A soft assertion is a quality score with a numeric threshold. If the score falls below the threshold, the eval becomes `scored` rather than `passed` — a signal that there is a quality regression, but not a hard breakage. Soft failures only count as failures when you run with `--strict`.
...
t.judge.closedQA("Is the tone professional?").atLeast(0.7);  // soft, threshold 0.7
t.maxTokens(80_000).soft();                       // demote to soft
```

## Verdict rules

Once all assertions are collected, `verdict.ts` folds them into a single verdict in this order:
...
All gates passed, but at least one soft is below its threshold →  scored
Otherwise                                                       →  passed
```
...
## 1. Value assertions — `fasteval/expect` matchers
...
The matchers available from `fasteval/expect`:
...
t.diff.isEmpty();                      // no repository files were modified this turn
t.notInDiff(/sk-[A-Za-z0-9]/);         // diff contains no secrets / inline styles
t.testsPassed();                       // EVAL.ts ran and all test() cases passed
t.scriptPassed("build");              // npm run build exited 0
t.noFailedShellCommands();
```

`t.diff` is a queryable object. `t.diff.get("src/Button.tsx")` returns the file's post-change content; `t.diff.isEmpty()` checks whether any files changed; `t.diff.matches(re)` and `t.notInDiff(re)` run a regex over the full diff text.
...
```ts
t.judge.factuality(reference).atLeast(0.8);   // factual consistency with a reference text
t.judge.closedQA("Is the response appropriate for a 10-year-old?");
t.judge.summarizes(sourceDocument);            // faithful summarization check
t.judge.score("Custom scoring rubric description", { on: t.reply });
```
...
defineEval({
  ...
  async test(t) {
    t.judge.factuality(reference);  // uses claude-opus-4-8 for this eval
  },
});

// A single call override
t.judge.score("rubric", { on: t.reply, model: "openai/gpt-4o" });
```
...
You can also assert **agent behavior** (not just output files) by reading the observability summary that fasteval injects into the sandbox:
...
t.maxTokens(80_000).soft();     // soft variant — tracked but only red under --strict
...
Custom matchers compose with the same chain methods as built-ins: `.gate()`, `.soft()`, `.atLeast(threshold)`.

## Related pages

- [Evals](/concepts/evals) — how assertions fold into the eval lifecycle and verdict types.
