...
export default defineEval({
  ...
  async test(t) {
    await t.sandbox.uploadDirectory("../workspaces/button");
    ...
    const build = await t.sandbox.runCommand("npm", ["run", "build"]);
...
Run the coding-agent eval. The `button` eval's experiment (or `niceeval.config.ts`) must already set `sandbox: dockerSandbox()` as shown above — niceeval no longer picks a default backend:

```shell
export ANTHROPIC_API_KEY=sk-ant-...
npx niceeval exp local button
```
