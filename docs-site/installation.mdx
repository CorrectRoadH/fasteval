...
export default defineConfig({
  ...
  // project-wide fallback sandbox backend for coding-agent evals;
  // an experiment's own `sandbox` field takes precedence over this
...
| `sandbox` | `SandboxSpec` | Sandbox backend for coding-agent evals, built with `dockerSandbox()` / `vercelSandbox()` / `e2bSandbox()` from `niceeval/sandbox`. Required for sandbox agents — either here as a project-wide fallback or on the experiment itself (`exp.sandbox ?? config.sandbox`); niceeval no longer auto-detects a backend |
