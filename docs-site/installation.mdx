...
export default defineConfig({
  ...
  // sandbox omitted here: uses a cloud sandbox token if present, otherwise Docker
});
```
...
| `sandbox` | `SandboxSpec` | Sandbox backend for coding-agent evals, built with `dockerSandbox()` / `vercelSandbox()` / `e2bSandbox()` from `niceeval/sandbox`; omit to auto-detect |
