---
title: "defineEval: 声明、配置并运行 fasteval eval"
sidebarTitle: "defineEval"
description: "defineEval 和 defineAgentEval 参考：选项、test context t、Turn 返回值和数据集数组导出。"
---
...
<Note>
  不要提供 `id` 或 `name`。fasteval 从文件路径推导 eval ID。
</Note>
...
<ParamField body="t.testsPassed()" type="function">
  断言 sandbox 验证测试通过。
</ParamField>
...
```ts
t.judge.factuality(expected, { on: t.reply }).atLeast(0.8);
t.judge.closedQA(question, { on: t.reply }).atLeast(0.7);
t.judge.score(rubric, { on: t.reply }).atLeast(0.75);
```
