...
<ParamField body="sandbox" type="SandboxSpec">
  默认 sandbox 后端，用 `niceeval/sandbox` 里的 `dockerSandbox()` / `vercelSandbox()` / `e2bSandbox()` 构造，不再接受 `"docker"` 这类裸字符串；省略该字段则从环境变量自动探测后端，或用 `--sandbox` 在 CLI 按名字指定。起始文件不会自动上传；sandbox eval 在 `test(t)` 里调用 `t.sandbox.uploadDirectory(...)`、`uploadFiles(...)` 或 `writeFiles(...)`。
</ParamField>
