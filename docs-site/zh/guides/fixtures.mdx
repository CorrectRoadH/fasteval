...
评估 coding agent 时，单纯检查一段回复不够。你通常要给它一个真实项目，让它读写文件、运行命令、提交改动，然后检查产物是否正确。niceeval 的当前推荐写法是：用普通 `.eval.ts` 文件显式准备 sandbox workspace、发送任务并断言结果。
...
export default defineEval({
  ...
  async test(t) {
    ...
    await t.send("在 src/routes/users.ts 里实现 POST /users 路由。").then((turn) => turn.expectOk());
...
`t.fileChanged()` 是作用域断言：它在运行结束后用 sandbox diff 判断目标文件是否真的被修改。
...
如果只是调用 HTTP 服务或进程内函数，用普通 direct adapter 会更轻。
