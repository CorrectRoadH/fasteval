---
title: "在 GitHub Actions 和 CI 中运行 fasteval"
sidebarTitle: "CI 集成"
description: "把 fasteval 接入 GitHub Actions 或任意 CI。eval 失败时非零退出，输出 JUnit XML，并通过缓存加速重复运行。"
---
...
默认情况下，只要存在失败的 gate，fasteval 就应该让命令非零退出。CI 中通常使用 `--strict`，让失败更明确。
...
可以在 CI 中缓存 `.fasteval/`，但要确保 fingerprint 覆盖了影响结果的输入。对于 nightly 基准，通常保留完整 artifacts 更有价值。
...
<CardGroup cols={2}>
  ...
  <Card title="Nightly full matrix" icon="moon">
    跑完整 experiment，记录 pass rate、成本和延迟趋势。
  </Card>
</CardGroup>
