...
sidebarTitle: "评分指南"
description: "用 fasteval 的五种评分机制评估任意 eval：值断言、作用域断言、LLM-as-judge、test-as-scoring 和效率检查。"
---

好的 eval 应该尽量把“是否成功”拆成可解释的信号。fasteval 允许你混合精确断言、语义 judge、事件流检查和真实测试。
...
| 输出是否语义正确 | `t.judge.*` |
| 代码是否工作 | `EVAL.ts` 或 `t.testsPassed()` |
| 成本是否可控 | `t.maxTokens` / `t.maxCost` |

## 值断言

```ts
import { includes, equals, matches } from "fasteval/expect";
...
```ts
t.judge.factuality("Refunds are available within 30 days.", { on: t.reply }).atLeast(0.8);
t.judge.score("The answer is specific, polite, and does not invent policy.", { on: t.reply }).atLeast(0.75);
```
...
t.check(t.reply, includes("friendly tone").soft());
```
