...
选择后端只能写在代码里——没有对应的 CLI flag，niceeval 也不会自动探测后端。在 experiment 里设置 `sandbox` 字段（或者在 `niceeval.config.ts` 里设置作为项目级兜底），三选一：
...
如果 experiment 和 `niceeval.config.ts` 都没有设置 `sandbox`，niceeval 在创建 sandbox 时会直接报错，而不会猜一个后端。
