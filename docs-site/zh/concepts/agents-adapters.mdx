...
description: "了解 niceeval 如何通过命名 agent adapters 连接任意 AI。远程 agent 包装 HTTP 或进程内调用，sandbox agent 在隔离环境中运行 CLI。"
---
...
niceeval 内置 `claude-code`、`codex` 和 `bub` 的 sandbox adapters。自定义 adapter 用 `defineAgent` / `defineSandboxAgent` 创建,再从 experiment 文件引用。三者具体做到哪一档能力（事件流、多轮会话、tracing…）见 [内置 Agent 分档参考](/zh/reference/builtin-agents)。
