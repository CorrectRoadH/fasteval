---
title: "fasteval scoring: 断言、judge 和 verdict"
sidebarTitle: "Scoring"
description: "fasteval 的五种评分机制：值断言、作用域断言、LLM-as-judge、test-as-scoring 和效率检查，以及 gate / soft 规则。"
---

fasteval 的评分目标是把 agent 行为变成可解释、可比较的结果。你可以混合使用精确断言、事件流断言、LLM judge、沙箱测试和成本约束。
...
<Tabs>
  <Tab title="gate">
    gate 是硬门槛。任何 gate 失败都会让 eval 的 verdict 变成 `failed`。
  </Tab>
  <Tab title="soft">
...
`fasteval/expect` 提供可组合 matcher：

```ts
import { includes, equals, matches, similarity } from "fasteval/expect";
...
```ts
t.judge.closedQA("回答是否准确说明了退款政策?", { on: t.reply }).atLeast(0.7);
t.judge.summarizes(sourceText, { on: t.reply }).atLeast(0.8);
t.judge.score("礼貌、具体、没有编造事实", { on: t.reply }).atLeast(0.75);
```
...
测试输出会进入 `.fasteval/<run>/.../test-output.txt`。
...
- [Evals](/zh/concepts/evals) — 断言如何进入生命周期和 verdict。
