---
title: "fasteval 中的 eval: 生命周期、verdict 与文件"
sidebarTitle: "Evals"
...
<Note>
  不要手写 `id` 或 `name`。fasteval 从文件路径推导 ID。
</Note>

## 路径即身份

`evals/weather/brooklyn.eval.ts` 的 ID 是 `weather/brooklyn`。位置参数按 ID 前缀过滤：

```bash
npx fasteval weather
npx fasteval weather/brooklyn
```
...
<Steps>
  ...
  <Step title="Scoring">
    fasteval 收集值断言、作用域断言、judge 分数和测试结果。
  </Step>
  <Step title="Verdict">
    所有断言结果折叠成一个最终 verdict。
  </Step>
  <Step title="Report">
    控制台和 reporters 输出结果，同时写入 `.fasteval/` artifacts。
  </Step>
</Steps>
...
生成 ID 类似 `sql/0000`、`sql/0001`。详见 [数据集扇出](/zh/guides/dataset-fanout)。
