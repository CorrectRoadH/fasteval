---
title: "How the fasteval runner schedules and executes evals"
sidebarTitle: "Runner"
description: "The fasteval runner discovers evals, schedules them with bounded concurrency, caches results, retries flaky infrastructure, and enforces budget guardrails."
---

The fasteval runner is the scheduling engine that turns a batch of evals into a finished result set. It handles everything that is the same across every eval — discovery, bounded concurrency, retry, early-exit, caching, reporting, and exit codes — while remaining completely indifferent to how any individual agent or scorer works. The runner drives every agent through the unified `send` verb and never branches on agent names or sandbox backends.
...
npx fasteval --tag regression
```

```bash Combined
# Only evals under "fixtures/" tagged "smoke"
npx fasteval fixtures --tag smoke
```
</CodeGroup>

<Tip>
  The positional argument is an **ID prefix**, not a glob pattern. Passing `weather` matches `weather/brooklyn`, `weather/tokyo`, and any other eval whose ID starts with `weather`.
</Tip>
...
run:setup           { }                         # hooks.run.setup begins
...
run:teardown        { }                         # hooks.run.teardown begins
run:saved           { outputDir }
run:summary         { passed, failed, scored, skipped, errored, durationMs, usage, estimatedCostUSD }
```

Failure events (`run:setupFailed`, `attempt:teardownFailed`, `run:teardownFailed`) are also emitted when hooks encounter errors.

## Lifecycle hooks

Environment setup and teardown are handled through a layered hooks system. All hooks live in a single `hooks` object with consistent `setup` / `teardown` verbs, scoped by a structural key:

<Accordion title="hooks.run.setup / hooks.run.teardown — run scope">
  These run **once per run**: `setup` fires before the first attempt, `teardown` fires after the last. Use them to start and stop shared resources like mock APIs, shared databases, or warm pools.

  If `hooks.run.setup` throws, the **entire run is aborted**.
</Accordion>

<Accordion title="hooks.sandbox.setup / hooks.sandbox.teardown — per-attempt scope">
  These run **once per attempt**: `setup` prepares the sandbox before each eval (writing `.env` files, starting services, installing dependencies), and `teardown` cleans up afterward.

  `setup` can read `ctx.flags` to branch on feature flags, and it may return a cleanup closure. If `hooks.sandbox.setup` throws, that eval is marked failed — other evals are unaffected. `teardown` always runs in `finally` and its errors are recorded as diagnostics only, never changing the verdict.
</Accordion>

Config-level hooks and experiment-level hooks are merged, with teardown running in reverse order. Lifecycle hooks manage **resource lifecycle**; reporters handle **result consumption** — the two are orthogonal.
...
| Any eval `scored` with `--strict` enabled | Non-zero |
