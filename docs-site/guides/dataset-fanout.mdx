...
Sometimes one eval scenario isn't enough — you want to verify that your agent handles dozens of inputs correctly, with each case getting its own pass/fail verdict in the report. fasteval's **dataset fan-out** lets you express this without creating dozens of individual files. When you export an array from a `.eval.ts` file, fasteval treats each element as a separate eval, assigns it a stable ID, and runs them all with the same concurrency and reporting as any other eval suite.

## How fan-out works

A normal `.eval.ts` file exports a single `defineEval`. A dataset file exports an **array** of `defineEval` calls. fasteval detects the array and creates one eval per element:
...
Writing every test case inline gets unwieldy fast. Use `loadYaml` or `loadJson` from `fasteval/loaders` to read cases from external files, then map them into `defineEval` calls:
...
```ts
import { loadJson } from "fasteval/loaders";

const cases = await loadJson("evals/data/sql-cases.json");
```

<Note>
  The path you pass to `loadYaml` / `loadJson` is resolved relative to the project root (where `fasteval.config.ts` lives), not relative to the eval file. Use paths like `evals/data/my-dataset.yaml`.
</Note>
...
npx fasteval sql/0000

# Run cases 0–2
npx fasteval sql/0000 sql/0001 sql/0002
```
...
import { defineEval } from "fasteval";
import { loadYaml } from "fasteval/loaders";
import { equals, includes } from "fasteval/expect";
...
export default rows.map((row) =>
  defineEval({
    ...
    async test(t) {
      ...
      t.judge.closedQA("Is this valid, syntactically correct SQL?").atLeast(0.8);
    },
  }),
);
```
