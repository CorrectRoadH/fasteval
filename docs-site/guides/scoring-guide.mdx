...
description: "Use fasteval's five scoring mechanisms — value assertions, scoped assertions, LLM-as-judge, test-as-scoring, and efficiency checks — to grade any eval."
---

Scoring is the process of folding a run's results into a **verdict**. fasteval provides five complementary mechanisms, each suited to different kinds of evidence: precise value checks, behavioral observations, open-ended quality judgments, test suite outcomes, and efficiency budgets. Every mechanism produces a named `Assertion` with a severity and a score. At the end of a run, fasteval aggregates all assertions into a single verdict. This guide covers all five mechanisms in depth, plus how to write custom assertions and how to control severity with `.soft()` and `.gate()`.

## Gate vs soft: how severity works

Every assertion has a **severity** that determines how a failure affects the verdict:

- **`gate`** — A hard requirement. One failed gate assertion marks the entire eval as `failed`. Use gates for facts that must be true.
- **`soft`** — A quality signal with a threshold. Falling below the threshold downgrades the verdict to `scored` rather than `failed`. Scored evals only turn red under `--strict`. Use soft for continuous quality dimensions like similarity or judge scores.

Most matchers default to `gate`. LLM-as-judge and `similarity` default to `soft`. You can always override with the chainable `.soft()` and `.gate()` methods:

```ts
t.check(t.reply, includes("regards"));            // gate by default
t.check(t.reply, similarity(expected).gate());    // force gate
t.judge.closedQA("Polite?").atLeast(0.7);         // soft with threshold 0.7
t.maxTokens(80_000).soft();                        // downgrade to soft
```
...
Value assertions evaluate a specific value **in place**, at the moment you call them. Import matchers from `fasteval/expect` and pass them to `t.check()` or `t.require()`.
...
t.diff.isEmpty();                        // No files changed at all
t.diff.get("src/Button.tsx");            // Returns the post-change file contents (use with t.check)
t.notInDiff(/sk-[A-Za-z0-9]/);          // Diff text doesn't match this pattern (e.g. no secrets)
t.testsPassed();                         // EVAL.ts Vitest tests all passed
t.scriptPassed("build");                 // npm run build exited 0
t.noFailedShellCommands();               // No shell command exited non-zero
```

`t.diff` is a queryable object. `t.diff.get("src/Button.tsx")` returns the post-change contents of a file — use it with `t.check` to assert on file content. `t.diff.isEmpty()` returns true if no files changed. `t.diff.matches(re)` and `t.notInDiff(re)` test the full unified diff string.
...
```ts
t.judge.factuality(expectedFact).atLeast(0.8);           // Is the reply factually consistent?
t.judge.closedQA("Is this appropriate for a 10-year-old?"); // Closed yes/no judgment
t.judge.summarizes(sourceDocument);                       // Does the reply faithfully summarize this?
t.judge.score("Rate how concise and direct the answer is", { on: t.reply });
```
...
t.judge.closedQA("Is the tone professional and confident?", { on: draft.message });
```
...
3. The global `judge.model` in `fasteval.config.ts`

```ts
// fasteval.config.ts — global default
...
export default defineEval({
  ...
  async test(t) {
    t.judge.factuality(expected);  // Uses claude-opus-4-8
  },
});
```
...
t.judge.score("Rate technical accuracy", { model: "openai/gpt-4o" });
```
...
```ts
t.scriptPassed("build");    // Assert that npm run build exits 0
t.testsPassed();            // Assert that all EVAL.ts tests pass
```
...
Token usage is a first-class scoring dimension in fasteval. An agent that answers correctly but uses ten times more tokens than necessary shouldn't score identically to one that answers efficiently. Usage data is collected automatically from the run's transcript.

```ts
t.maxTokens(50_000);            // Hard limit: > 50k tokens → failed (gate by default)
t.maxCost(0.50);                // Hard limit: estimated cost > $0.50 → failed
t.maxTokens(80_000).soft();     // Soft limit: only fails under --strict
```
...
<Note>
  `t.maxCost()` requires a price table to be configured so fasteval can estimate costs from token counts. Check your `fasteval.config.ts` for cost configuration options.
</Note>
...
A value assertion is just a function `(value) => number | Promise<number>`. Use `makeAssertion` from `fasteval/expect` to wrap any scoring logic into a reusable matcher:

```ts
import { makeAssertion } from "fasteval/expect";
import type { Assertion } from "fasteval/expect";
...
Custom matchers follow the same `.soft()` / `.gate()` / `.atLeast(n)` chaining as built-in matchers. Export them from a shared file to reuse them across multiple evals.
...
After all assertions are collected, fasteval folds them into a verdict in this order:
...
All gates passed, but a soft is below its threshold    → scored   (only fails under --strict)
Otherwise                                              → passed
```
...
| `scored` | All gates passed, but at least one soft fell below its threshold |
| `skipped` | `t.skip(reason)` was called |

When you run an eval multiple times with `--runs N`, the suite-level result is a **pass rate** (fraction of runs that passed) and average duration, rather than a single verdict.
