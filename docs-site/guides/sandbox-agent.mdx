...
description: "Use niceeval's built-in agents (claude-code, codex, bub) or write a custom adapter to run a coding-agent CLI in an isolated Docker or cloud sandbox."
---
...
Select the agent in an experiment file, and set `sandbox` on that same experiment (or as a project-wide fallback in `niceeval.config.ts`) to pick where niceeval spins up the isolated environment (see [Sandbox Backends](/guides/sandbox-backends) for the full list of options).

```ts
// experiments/local.ts
import { defineExperiment } from "niceeval";
import { dockerSandbox } from "niceeval/sandbox";
import claudeCode from "niceeval/agents/claude-code";
...
<Note>
  There is no CLI flag for choosing a sandbox backend — sandbox backend selection lives entirely in code. If neither the experiment nor `niceeval.config.ts` sets `sandbox`, niceeval throws as soon as it tries to create one.
</Note>
...
niceeval provides helpers that sandbox adapters can reuse for CLI setup, transcript capture, and transcript parsing. Workspace preparation and validation usually live in the eval's `test(t)` function; diff collection is handled by the runner.

| Helper | What it does |
|---|---|
| `shared.ensureInstalled(sandbox, command, install)` | Ensures an agent CLI exists in the sandbox |
| `shared.writeFile(sandbox, path, content)` | Writes config or helper files into the sandbox |
| `shared.captureLatestJsonl(sandbox, dir)` | Finds and reads the most recent JSONL transcript in the given directory |
| `shared.parseClaudeCode(raw)` | Parses Claude Code JSONL into standard events and usage |
| `shared.parseCodex(raw)` | Parses Codex JSONL into standard events and usage |
| `shared.parseBub(raw)` | Parses bub JSONL into standard events and usage |
...
```shell
npx niceeval exp local fixtures/my-task
```
