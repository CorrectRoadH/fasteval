...
fasteval ships three sandbox adapters out of the box. You select them with `--agent <name>`; no configuration file entry is required.
...
Pass `--agent` and `--sandbox` on the CLI. The sandbox flag tells fasteval where to spin up the isolated environment (see [Sandbox Backends](/guides/sandbox-backends) for the full list of options).
...
<Note>
  The `--sandbox docker` flag is optional if Docker is your default backend. Run `npx fasteval --agent codex fixtures/button` and fasteval will use Docker automatically unless a cloud token is detected in the environment.
</Note>
...
The runner prepares the environment, then hands control to the adapter for exactly one segment. Everything else — uploading files, committing a git baseline, running validation, collecting diffs — is handled by fasteval's shared infrastructure so that results are consistent across all agents.

```
createSandbox(backend, timeout)
  → uploadFiles(workspaceFiles)        # EVAL.ts is hidden from the agent
  → git init && git commit             # baseline for later diff
  → hooks.sandbox.setup?(sandbox, ctx) # optional user hook
  → npm install                        # fixture dependencies
  → adapter.send(input, ctx)           # ← the adapter's only segment
  → uploadFiles(testFiles)             # EVAL.ts appears now
  → runValidation()                    # run EVAL.ts + npm scripts
  → collectGeneratedFiles()            # git diff HEAD
  → hooks.sandbox.teardown?()/cleanup() # user teardown hook (runs even on failure)
...
| `shared.captureGeneratedFiles(sandbox)` | Runs `git diff HEAD` and returns `{ generated, deleted }` — the file-level diff used for `t.fileChanged`, `t.fileDeleted`, and `t.diff` |
...
Built-in agents (`claude-code`, `codex`, `bub`) require no configuration — they are available immediately via `--agent <name>`. Custom adapters must be added to `fasteval.config.ts`:
...
Then run it exactly like a built-in:

```shell
npx fasteval --agent my-custom-agent --sandbox docker fixtures/my-task
```
