...
export default defineEval({
  ...
  async test(t) {
    ...
    await t.send("Implement POST /users in src/routes/users.ts.").then((turn) => turn.expectOk());
...
`t.fileChanged()` is a scoped assertion. It is evaluated from the sandbox diff after the run.
...
If the system under test is an HTTP service or in-process function, a direct adapter is lighter.
