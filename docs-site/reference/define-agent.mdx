...
<ResponseField name="runCommand(cmd, args?, opts?)" type="(cmd: string, args?: string[], opts?: RunOpts) => Promise<CommandResult>">
  ...
  ```ts
  const res = await ctx.sandbox.runCommand("npm", ["install"]);
  ```

  **`opts` fields:**
  - `env?: Record<string, string>` — extra environment variables merged into the command's environment
  - `cwd?: string` — working directory override for this command. Relative paths resolve under `ctx.sandbox.workdir`; omitted commands run in `workdir`.
...
<ResponseField name="workdir" type="string">
  The sandbox's default workspace directory. Relative sandbox paths, omitted upload targets, and omitted command `cwd` values resolve here. Use it only when you need to pass an absolute path to an agent prompt or debug output.
</ResponseField>
...
<ResponseField name="writeFiles(files, targetDir?)" type="(files: Record<string, string>, targetDir?: string) => Promise<void>">
  ...
  contents. If `targetDir` is omitted, files are written under `workdir`.
...
<ResponseField name="uploadFiles(files, targetDir?)" type="(files: SandboxFile[], targetDir?: string) => Promise<void>">
  Uploads a batch of files, including binary files. If `targetDir` is omitted, files are uploaded under `workdir`.
</ResponseField>
...
```shell
npx niceeval exp claude-code-local evals/fixtures/button
npx niceeval exp codex-local evals/fixtures/button
```

Each of these experiments must set `sandbox: dockerSandbox()` (or `vercelSandbox()` / `e2bSandbox()`) itself — niceeval no longer picks a default backend, and there is no CLI flag for choosing one.
