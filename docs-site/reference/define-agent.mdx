...
<ParamField body="name" type="string" required>
  A unique identifier for this agent. Referenced by name in `defineEval({ agent: "..." })`,
  `defineConfig({ defaultAgent: "..." })`, and the `--agent` CLI flag.
...
<ParamField body="capabilities" type="AgentCapabilities">
  ...
  <ParamField body="workspace" type="boolean">
    The agent works on a file system. Enables `t.diff`, `t.fileChanged`,
...
<ResponseField name="usage" type="Usage | undefined">
  ...
  fasteval can report costs and power `t.maxTokens()` / `t.maxCost()` assertions.
...
<Note>
  ...
  read it from environment variables in the `send` closure. fasteval never sees
...
<ResponseField name="getWorkingDirectory()" type="() => string">
  Returns the current working directory path inside the sandbox.
</ResponseField>

<ResponseField name="setWorkingDirectory(path)" type="(path: string) => void">
...
The `shared` object from `fasteval` provides utilities that are common across all sandbox agent adapters, ensuring that workspace preparation, diff collection, validation, and observability injection work consistently regardless of which agent CLI you're wrapping.
...
<ResponseField name="shared.injectO11yContext(sandbox, events)" type="function">
  ...
  `__fasteval__/results.json` inside the sandbox. This makes agent behavior
...
## Registering agents in defineConfig

Once you've written your adapters, register them in `fasteval.config.ts`:
...
export default defineConfig({
  ...
  defaultAgent: "my-agent",   // used when --agent is not specified
});
```

<ParamField body="agents" type="AgentAdapter[]">
  Array of agent adapter instances created with `defineAgent` or
  `defineSandboxAgent`. Each adapter's `name` must be unique.
</ParamField>

<ParamField body="defaultAgent" type="string">
  The `name` of the agent to use when neither the eval's `agent` field nor the
  `--agent` CLI flag specifies one.
</ParamField>

### Built-in agents

The following coding agent adapters are built into fasteval and require no registration. Use them directly with `--agent`:
...
```shell
npx fasteval --agent claude-code --sandbox docker evals/fixtures/button
npx fasteval --agent codex --sandbox docker evals/fixtures/button
```
...
| Sandbox | `ctx.sandbox` (raw `Sandbox` handle) | `t.diff`, `t.fileChanged`, etc. (high-level view) |
