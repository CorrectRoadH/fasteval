---
title: "defineEval: declare, configure, and run evals in fasteval"
sidebarTitle: "defineEval"
description: "Complete reference for defineEval and defineAgentEval. Covers all options, the test context t, Turn return value, and dataset array exports."
---

`defineEval` is the primary building block for writing evals in fasteval. You call it once per file, pass a configuration object describing your eval, and export the result as the default export. The runner discovers your file, derives its ID from the file path, and executes the `test` function against the configured agent.
...
<Note>
  You must **not** provide an `id` or `name` field. fasteval derives the eval ID
...
<ParamField body="agent" type="string">
  Name of the agent adapter to use for this eval. Must match a `name` registered
  in `defineConfig({ agents: [...] })`. When omitted, fasteval falls back to
  `defaultAgent` in your config, or the value of `--agent` on the CLI.

  ```ts
  export default defineEval({
    agent: "my-agent",  // references a named agent adapter
    async test(t) { /* ... */ },
  });
  ```
</ParamField>
...
<ParamField body="test" type="(t: TestContext) => Promise<void>" required>
  ...
  async test(t) {
    ...
    t.judge.summarizes(document).atLeast(0.8);
...
<ParamField body="t.check(value, assertion)" type="(value: unknown, assertion: Assertion) => void">
  ...
  → `failed`, `soft` → `scored`). Does **not** throw — execution continues.
...
<ParamField body="t.log(msg)" type="(msg: string) => void">
  Writes a diagnostic message to the eval's output log. Appears in `.fasteval/`
...
<ParamField body="t.diff" type="DiffView">
  A queryable view of all changes the agent made to the workspace:

  - `t.diff.get(path)` — returns the post-run content of the file at `path`
  - `t.diff.isEmpty()` — asserts no files were changed
  - `t.diff.matches(re)` — asserts the full diff text matches a regex
  - `t.notInDiff(re)` — asserts the diff does **not** match the regex (useful for detecting leaked secrets or banned patterns)

  ```ts
  t.check(t.diff.get("src/Button.tsx"), includes("onClick"));
  t.notInDiff(/sk-[A-Za-z0-9]/); // no API keys in diff
  ```
</ParamField>

<ParamField body="t.testsPassed()" type="() => void">
  Asserts the `EVAL.ts` Vitest suite passed (all tests green). This is the
  primary gate assertion for fixture-based evals.
</ParamField>

<ParamField body="t.scriptPassed(scriptName)" type="(scriptName: string) => void">
...
<ParamField body="t.judge.factuality(expected, opts?)" type="(expected: string, opts?) => JudgeAssertion">
  ...
  ```ts
  t.judge.factuality("Paris is the capital of France.").atLeast(0.8);
  ```
</ParamField>

<ParamField body="t.judge.closedQA(question, opts?)" type="(question: string, opts?) => JudgeAssertion">
  ...
  ```ts
  t.judge.closedQA("Is the tone professional and on-topic?").atLeast(0.7);
  ```
</ParamField>

<ParamField body="t.judge.summarizes(source, opts?)" type="(source: string, opts?) => JudgeAssertion">
  Asks the judge whether the agent's reply faithfully summarizes the `source`
  text.
</ParamField>

<ParamField body="t.judge.score(rubric, opts?)" type="(rubric: string, opts?) => JudgeAssertion">
  ...
  ```ts
  t.judge.score("Does the answer use simple language suitable for a 10-year-old?", {
    on: turn.message,
  }).atLeast(0.6);
  ```
...
<ParamField body="t.maxTokens(n)" type="(n: number) => TokenAssertion">
  ...
  `n`. Defaults to `gate` severity. Chain `.soft()` to downgrade.

  ```ts
  t.maxTokens(50_000);          // gate: fails if over
  t.maxTokens(80_000).soft();   // soft: scored but not failed
  ```
</ParamField>
...
When a single eval file exports an **array** as its default export, fasteval fans it out into one eval per element. This is the canonical way to write parameterized test suites.
...
IDs are stable as long as the array order doesn't change, making them safe to reference in CI history, dashboards, and issue trackers. `loadJson` from `fasteval/loaders` works the same way as `loadYaml`.
