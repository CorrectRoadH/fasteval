...
<ParamField query="--sandbox" type="string">
  ...
  - `vercel` — use Vercel Sandbox (requires `VERCEL_API_TOKEN` and `VERCEL_TEAM_ID`).
  - `auto` — detect automatically: use `vercel` when Vercel credentials are present, otherwise fall back to `docker`.
