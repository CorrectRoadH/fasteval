...
> There is no CLI flag for choosing a sandbox backend. Sandbox backend selection is a written-in-code concern only — set `sandbox` in the experiment (`defineExperiment`) or as a project-wide fallback in `niceeval.config.ts` (`defineConfig`) to `dockerSandbox()` / `vercelSandbox()` / `e2bSandbox()` from `niceeval/sandbox`. Attempting to pass a sandbox flag on the CLI prints an error and exits 1.
...
<Tabs>
  <Tab title="Local development">
    ...
    npx niceeval exp local fixtures/button --watch
