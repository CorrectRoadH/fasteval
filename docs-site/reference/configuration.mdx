...
<ParamField body="sandbox" type="SandboxSpec">
  Default sandbox backend, built with `dockerSandbox()` / `vercelSandbox()` / `e2bSandbox()` from `niceeval/sandbox`; omit the field to auto-detect from env vars. Files are not uploaded automatically; sandbox evals call `t.sandbox.uploadDirectory(...)`, `uploadFiles(...)`, or `writeFiles(...)` in `test(t)`.
</ParamField>
