...
description: "Reference for project-wide niceeval defaults: judge, reporters, concurrency, timeout, and sandbox backend."
---
...
<ParamField body="sandbox" type="SandboxSpec">
  Default sandbox backend for sandbox agents, built with `dockerSandbox()` / `vercelSandbox()` / `e2bSandbox()` from `niceeval/sandbox` — bare strings like `"docker"` are no longer accepted here. Omit the field to auto-detect the backend from env vars. Override per experiment, or pick a backend by name at the CLI with `--sandbox`.
</ParamField>
