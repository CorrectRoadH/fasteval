...
- **背景调研:** [References(从其它项目学到什么)](references.md)。

## 关于这些文档

这些是**内部设计与实现协作文档**:记录 fasteval 的目标 DX、架构边界和实现协作约定。[Source Map](source-map.md) 用来帮助把设计行为定位到源码;如果代码实现与文档设计不一致,应进一步讨论并决定是修代码、修设计,还是记录为明确的阶段性差异。文档全部用中文书写。
