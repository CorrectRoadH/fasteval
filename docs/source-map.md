...
| `fromAiSdk`(通道 0:AI SDK 结果 → 标准事件流,v4/v5 字段漂移兜底) | `src/agents/ai-sdk.ts`(+ 同目录 `.test.ts`;用法见 `examples/zh/ai-sdk/`) |
...
- **judge 走 OpenAI 兼容 `/chat/completions`**,base/key 解析顺序:`judge.baseUrl/apiKeyEnv` → `NICEEVAL_JUDGE_BASE`/`CODEX_BASE_URL` → OpenAI 官方。这样在只有 OpenAI 兼容代理(无 Anthropic key)的环境里 judge 自动复用代理。
