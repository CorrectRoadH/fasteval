...
```
evals/        # 测什么 —— agent 无关,评分逻辑在各自 test() / EVAL.ts 里
...
| `validation` | — | **删** | 「怎么算对」是 eval 自己的事(`test()` / `EVAL.ts`),不该由 experiment 决定 |
...
- **`fasteval.config.ts`(`defineConfig`)** = 项目级默认:默认 sandbox、`judge`、`reporters`、`workspace`,以及项目级的[生命周期钩子](lifecycle.md)(`hooks.run` / `hooks.sandbox`,实验的同名钩子叠加在其上)。
