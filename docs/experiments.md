...
- **跨 agent / 跨配置对比是一等公民。** 评 coding agent 最想要的就是"一条命令,几个配置并列出**质量 × 成本**"(见 [Observability](observability.md#结果可视化niceeval-view))。niceeval 用**文件夹**表达"这一组该并排比"(见 [实验怎么组织](#实验怎么组织文件夹--一组可对比的实验));每个实验文件钉一个单一配置。
...
export default defineExperiment({
  ...
  sandbox?: SandboxSpec;                     // 沙箱型 agent 在哪跑,工厂函数产出(dockerSandbox() 等);省略 = auto 探测。`--sandbox` 可临时覆盖
...
flag 驱动的环境差异(比如按 `flags.skill` 往沙箱注入一个 skill 文件)写在 eval 的 `test(t)` 里:`if (t.flags.skill) await t.sandbox.writeFiles({ ".agent/skill.md": loadSkill(t.flags.skill) })`——普通代码,不需要框架钩子。
...
| `setup` | — | **删** | 环境预置不进 experiment:沙箱预置写 `test()`、连 agent 写 `SandboxAgent.setup`、整轮共享服务用外部编排(见 [环境预置放哪](sandbox.md#环境预置放哪)) |
...
| `onRunComplete` | — | **删** | 下游**分析**交给 [reporter](observability.md#reporters);**资源起停**不由 experiment 钩子管,靠外部编排 / `SandboxAgent.setup` / `test()`(见 [环境预置放哪](sandbox.md#环境预置放哪)) |
...
- **`niceeval.config.ts`(`defineConfig`)** = 项目级默认:`judge`、`reporters`、并发 / 超时、`pricing`。**沙箱后端不在 config** —— 由 experiment 的 `sandbox` 决定。
