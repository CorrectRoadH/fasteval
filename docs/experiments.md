...
flag 驱动的环境差异(比如按 `flags.skill` 往沙箱注入一个 skill 文件)写在 eval 的 `test(t)` 里:`if (t.flags.skill) await t.sandbox.writeFiles({ ".agent/skill.md": loadSkill(t.flags.skill) }, "/workspace")`——普通代码,不需要框架钩子。
...
- **`fasteval.config.ts`(`defineConfig`)** = 项目级默认:`judge`、`reporters`、并发 / 超时、`pricing`。**沙箱后端不在 config** —— 由 experiment 的 `sandbox` 决定。
